...
def check_support(test, source_release, target_release):
    ...
    """
    source_major_release = int(source_release.partition(".")[0])
...
def prepare_repos_in_guest(test, params, vm, target_release):
    ...
    tmp_upgrade_repos_path = os.path.join(tmp_dir, "tmp_leapp_upgrade_repositories.repo")
    ...
    os.remove(tmp_upgrade_repos_path)
...
def verify_upgrade_succeeded(test, params, vm, session, target_release, target_page_size):
    ...
    if target_page_size != post_page_size:
        ...
        utils_test.update_vm_default_kernel(vm, kernel_version, timeout=900, guest_arch_name=vm_arch_name)
...
def get_kernel_version(test, params, session, target_release, target_page_size):
    ...
    try:
        ...
        test.log.info("A kernel with version %s and pagesize %s could be found" %
                      (release, page_size))
        test.log.debug("kernel path found: %s; kernel version: %s" % (kernel, kernel_version))
