...
def create_custom_loader(test, loader_path, new_custom_path):
    try:
        ...
        logging.info("ovmf lodader copyed to the custom path {}".format(new_custom_path))
...
def run(test, params, env):
    ...
    try:
        ...
        if os.path.isfile(custom_loader_path):
            os.remove(custom_loader_path)
            logging.debug("cleaned up %s", custom_loader_path)
