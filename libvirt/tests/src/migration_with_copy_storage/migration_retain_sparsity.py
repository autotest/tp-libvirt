...
def prepare_disks_local(params, vm_name):
    ...
    src_driver_type_2 = params.get("src_driver_type_2")
    ...
    if src_driver_type_1 == "rbd":
        ceph.rbd_image_rm(mon_host, ceph_pool_name, src_disk1_name)
        ceph.rbd_image_create(mon_host, ceph_pool_name, src_disk1_name, disk_size)
        disk1_path = ceph.rbd_image_map(mon_host, ceph_pool_name, src_disk1_name)
        test.log.debug("for test: disk1_path: %s" % disk1_path)
    ...
    if src_driver_type_2:
        src_disk2_dict = eval(params.get("src_disk2_dict"))
        _prepare_disk_xml(disk2_name, src_driver_type_2, src_disk2_dict)
...
def prepare_disks_remote(params):
    ...
    dest_driver_type_2 = params.get("dest_driver_type_2")
...
def prepare_migratable_xml(vm_name, params, test):
    ...
    """
    dest_device_type_1 = params.get("dest_device_type_1")
    dest_device_type_2 = params.get("dest_device_type_2")
    dest_disk1_dict = eval(params.get("dest_disk1_dict"))

    mig_disk1 = disk.Disk(type_name=dest_device_type_1)
    mig_disk1.setup_attrs(**dest_disk1_dict)
    if dest_device_type_2:
        dest_disk2_dict = eval(params.get("dest_disk2_dict"))
        mig_disk2 = disk.Disk(type_name=dest_device_type_2)
...
def run(test, params, env):
    ...
    def setup_test():
        ...
        prepare_disks_local(params, vm_name)
        prepare_disks_remote(params)
        vm.start()
        vm_session = vm.wait_for_serial_login(timeout=120)
        vm_session.cmd(f"dd if=/dev/zero of=/dev/{target_dev} bs=512 count=2097152")
        vm_session.cmd(f"mkfs.xfs /dev/{target_dev}")
        vm_session.cmd("dd if=/dev/random of=/media/data bs=1048576 count=100")
        vm_session.close()

        nonlocal old_disk_size
        if src_driver_type_1 == "rbd":
            ret = process.run(f"rbd du --pool={ceph_pool_name}", shell=True).stdout_text.strip()
            test.log.debug(f"for test: {ret}")
        else:
            old_disk_size = utils_misc.get_image_info(os.path.join(disk_path, src_disk1_name))["dsize"]
    ...
    def verify_test():
        ...
        if dest_driver_type_1 == "rbd":
            cmd = f"rbd du --pool={ceph_pool_name}"
            ret = remote.run_remote_cmd(cmd, params).stdout_text.strip()
            test.log.debug(f"for test: new {ret}")
        else:
            disk_file = os.path.join(disk_path, dest_disk1_name)
            cmd = f"qemu-img info {disk_file} -U"
            ret = remote.run_remote_cmd(cmd, params).stdout_text.strip()
            new_disk_size = ret.split('\n')[3].split(':')[1].strip().split(' ')
            test.log.debug(f"new disk size: {new_disk_size}")
            if new_disk_size[1] == "MiB":
                if float(new_disk_size[0]) * 1024 * 1024 == old_disk_size:
                    test.fail(f"Disk usage should not be {old_disk_size}")
            elif new_disk_size[1] == "GiB":
                if new_disk_size[0] == old_disk_size:
                    test.fail(f"Disk usage should not be {old_disk_size}")
            else:
                test.fail(f"Disk size error.")
    ...
    def cleanup_test():
        ...
        if src_driver_type_1 == "rbd":
            ceph.rbd_image_unmap(ceph_pool_name, src_disk1_name)
            ceph.rbd_image_rm(params.get("mon_host"), ceph_pool_name, src_disk1_name)
        else:
