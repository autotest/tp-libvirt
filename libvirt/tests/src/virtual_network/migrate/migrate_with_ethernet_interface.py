...
def run(test, params, env):
    ...
    """
    def create_unpr_user_and_vm(user_name, user_passwd, main_vm_name):
        """
        ...
        """
        test.log.info(f"Creating unprivileged user: {user_name}")

        # Create unprivileged user with password
        try:
            process.run(f'useradd -m {user_name}', shell=True, ignore_status=True)
            process.run(f'echo "{user_name}:{user_passwd}" | chpasswd', shell=True)
    ...
    virsh_migrate_connect_uri = params.get("virsh_migrate_connect_uri")

    test.log.debug("222222222222222222222:%s", dest_uri)

    vmxml = vm_xml.VMXML.new_from_inactive_dumpxml(vm_name)
    bk_xml = vmxml.copy()

    unpr_vm_obj = create_unpr_user_and_vm(unpr_user, unpr_passwd, vm_name)

    migration_obj = base_steps.MigrationBase(test, unpr_vm_obj, params)
    virsh_ins = virsh.Virsh(uri=f'qemu+ssh://{unpr_user}@localhost/session')
