...
def get_init_iface_info(params, test, item):
    """
    Get guest initial interface info by item.

    :param params: Test parameters dictionary containing:
                  - main_vm: VM name to check
                  - status_error: Boolean indicating if the operation should fail
...
def run(test, params, env):
    ...
    default_iface_alias = get_init_iface_info(params, test, 'iface_alias')
    default_iface_mac = get_init_iface_info(params, test, 'iface_mac')
    ...
    if params.get_boolean("add_pci"):
        pci_dict = get_init_iface_info(params, test, 'pci')
