...
def run(test, params, env):
    ...
    def define_testnet(net_name, subnet):
        """
        Define a virtual network with forward mode as 'nat'
        """
        virtual_net = f"""
    <network>
      <name>{net_name}</name>
    ...
    """
        try:
            test_xml = network_xml.NetworkXML()
    ...
    try:
        net_name1 = utils_misc.generate_random_string(8)
        subnet1 = params.get('subnet')
        logging.debug("TEST_STEP1: Define and start a network")
        define_testnet(net_name1, subnet1)
        virsh.net_start(net_name1, ignore_status=True)
        check_net_states(net_name1, True)
        logging.debug("TEST_STEP2: Delete the bridge to prepare an abnormal status")
        delete_bridge(net_name1)
        ...
        check_net_states(net_name1, False)
        logging.debug("TEST_STEP4: Start the network again and ensure it works well")
        ret = virsh.net_start(net_name1, ignore_status=False)
        libvirt.check_result(ret)
    finally:
        virsh.net_destroy(net_name1, ignore_status=True)
        virsh.net_undefine(net_name1, ignore_status=True)
