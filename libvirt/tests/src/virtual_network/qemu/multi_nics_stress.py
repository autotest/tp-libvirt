...
def launch_netperf_client(
    ...
    while duration < max_run_time:
        ...
        for client in netperf_clients:
            status = client.is_netperf_running()
...
def run(test, params, env):
    ...
    netperf_server = params.get_list("netperf_server")
    ...
    ping_count = params.get_numeric("ping_count", 10)
    ...
    for server in netperf_server:
        ...
        if server in vms:
            ...
            server_ctl.cmd(stop_firewall, ignore_all_errors=True)
    ...
    if client in vms:
        ...
        if params.get("dhcp_cmd"):
            status, output = client_ctl.cmd_status_output(
                params["dhcp_cmd"], timeout=login_timeout
            )
            if status:
        ...
        client_ctl.cmd(stop_firewall, ignore_all_errors=True)
    ...
    if params.get("os_type") == "linux":
        ...
        for ip in server_ips:
            ...
            client_ctl.cmd("ip route add %s dev %s" % (ip, nics_list[index]))
    ...
    try:
        ...
        for s_ip in server_ips:
            ...
            if package_lost != 0:
                test.fail(
                    "%s packets lost when pinging server IP %s" % (package_lost, s_ip)
                )
    finally:
        for n_server in netperf_servers:
            n_server.stop()
