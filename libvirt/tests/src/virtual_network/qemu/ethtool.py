...
def run(test, params, env):
    ...
    def setup_test():
        ...
        """
        test.log.info("TEST_SETUP: Setup network and start VM.")

        # Setup bridge if needed
        if params.get("br_scenario") == "linux_bridge":
            utils_net.create_linux_bridge_tmux(bridge_name, host_iface)

        elif params.get("br_scenario") == "ovs_bridge":
            libvirt_network.create_or_del_network(net_attrs)
            status, stdout = utils_net.create_ovs_bridge(bridge_name, ip_options='-color=never')
            if status:
                test.fail("Failed to create ovs bridge on local. Status: %s"
                          "Stdout: %s" % (status, stdout))

        # Configure VM with the specified interface
        if iface_attrs:
    ...
    def teardown_test():
        ...
        if params.get("br_scenario") == "ovs_bridge":
            libvirt_network.create_or_del_network(net_attrs, is_del=True)
            utils_net.delete_ovs_bridge(bridge_name, ip_options='-color=never')
