...
#   Author: Wenli Quan<wquan@redhat.com>
#
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
from provider.virtual_network import netperf_base, vdpa_utils, network_base
from provider.windows import win_driver_utils
...
def run(test, params, env):
    ...
    """
    test.log.info("TEST_STEP 1: Start VM and prepare baseline for netperf")
    ...
    queues = params.get_numeric("queues", 1)
    server_private_ip = params.get("server_private_ip")
    host = params.get("host", "localhost")
    client = params.get("client", "localhost")
    mtu = params.get_numeric("mtu", "1500")
    os_type_client = params.get("os_type_client")
    ...
    def mtu_set(mtu):
        ...
        if os_type == "linux":
            ethname = utils_net.get_linux_ifname(server_ctl, data_mac)
        ...
        target_ifaces.extend(
            ovs_handler.get_vdpa_ovs_info(add_flows=False, return_ports=True)
        )
    ...
    test.log.info("TEST_STEP 2: Build/boot VM and wait for login")
    pinned_node = 0
    ...
    try:
        for node in node_list:
            node_mem_free = int(
                host_numa.read_from_node_meminfo(node, "MemFree"))
        ...
        if vm is None:
            env_process.process(
                test, params, env,
                env_process.preprocess_image, env_process.preprocess_vm
            )
            vm = env.get_vm(vm_name)
        recreate_serial_console = False
        if dev_type == "vdpa" and params.get("iface_dict"):
            ...
            iface_list = []
            if iface_dict:
                iface_list.append(("network", iface_dict))
            if iface_dict2:
                iface_list.append((iface_dict2.get("type_name", "vdpa"), iface_dict2))
            for iface_type, iface_dict in iface_list:
                mac = iface_dict.get("mac_address")
                if mac and params.get(mac):
                    iface_dict["mac_address"] = params.get(mac)
                iface_dev = interface_base.create_iface(iface_type, iface_dict)
                libvirt.add_vm_device(vmxml, iface_dev)
            vmxml.sync()
            if vm.virtnet:
                synced_xml = vm_xml.VMXML.new_from_dumpxml(vm_name)
                iface_devices = synced_xml.get_devices("interface")

                for i, iface_dev in enumerate(iface_devices):
                    if i < len(vm.virtnet) and getattr(iface_dev, "mac_address", None):
                        vm.virtnet[i].mac = iface_dev.mac_address
                        nettype = getattr(iface_dev, "type_name", "network")
                        if nettype == "network" and getattr(iface_dev, "source", {}).get("network"):
                            nettype = "bridge"
                        vm.virtnet[i].nettype = nettype
                        test.log.debug(f"Synced virtnet[{i}]: MAC={iface_dev.mac_address}, nettype={nettype}")
        if not vm.is_alive():
            vm.start()
        ...
        if f"node argument {pinned_node} is out of range" in str(e):
            test.cancel(
                f"The node: {pinned_node} used for VM pinning is not valid")
        test.error(e)

    test.log.info("TEST_STEP 3: Run guest configuration commands (if any)")
    ...
    if config_cmds:
        for config_cmd in config_cmds.split(","):
            ...
            if cmd:
                ...
                test.log.info(o)
                if "querysettings" in cmd and ".sys" in o:
    ...
    test.log.info(
        ...
        "(failover/driver setup if needed)"
    )
        ...
        if os_type == "windows" and params.get("install_vioprot_cmd"):
            ...
            for drv_name in driver_name.split():
                ...
                if driver_name == "VIOPROT":
                    test.log.info(
                        "Will install inf file found at '%s'", inf_path)
                ...
                session.cmd_output_safe(
                    "ipconfig /renew", timeout=login_timeout)
            session.close()
    else:
        try:
            vm.wait_for_serial_login(
                timeout=login_timeout,
                recreate_serial_console=recreate_serial_console,
    ...
    test.log.info(
        "TEST_STEP 5: Configure guest NIC (queues/IP) and collect addresses")
    mac = vm.get_mac_address(0)
    data_mac = vm.get_mac_address(1) if len(nics.split()) > 1 else mac
    if os_type == "linux":
        ethname = utils_net.get_linux_ifname(session, data_mac)
    queues = int(params.get("queues", 1))
    if queues > 1:
        if os_type == "linux":
            session.cmd_status_output(
                "ethtool -L %s combined %s" % (ethname, queues))
        else:
            test.log.info("FIXME: support to enable MQ for Windows guest!")

    server_ctl = vm.wait_for_login(nic_index=0, timeout=login_timeout)
    server_ctl_ip = network_base.get_vm_ip(server_ctl, mac)

    if server_private_ip and os_type == "linux":
        ...
        server_ip = network_base.get_vm_ip(server_ctl, data_mac)
    ...
    test.log.info(
        "TEST_STEP 6: Pin guest vCPU/vhost threads and setup host/client")
    # Pin guest vCPUs and vhost threads.
    # Use the specified NUMA node if provided; otherwise, pin sequentially to host CPUs.
    network_base.pin_vcpu_vhost_threads(vm, params.get("numa_node"))
    ...
    for i in range(2):
        if client != "localhost" and os_type_client == "linux":
            ...
            cmd = "ifconfig %s %s up" % (client_physical_nic, client_ip)
    ...
    test.log.info(
        "TEST_STEP 7: Prepare server/client/host environment (tuning/MTU)")
    prepare_list = set([server_ctl, client, host])
    tag_dict = {server_ctl: "server", client: "client", host: "host"}
    if client_pub_ip:
        ip_dict = {
            server_ctl: server_ctl_ip, client: client_pub_ip, host: host_ip}
    else:
    ...
    try:
        test.log.info("TEST_STEP 8: Run netperf tests and collect results")
        ...
        start_test(
            ...
            clients,
            test.resultsdir,
            test_duration=int(params.get("l", 60)),
        ...
        test.log.info(
            "TEST_STEP 9: Cleanup and restore environment/VM settings")
        if mtu != 1500:
            mtu_default = 1500
            error_context.context(
            ...
            mtu_set(mtu_default)
        if client_physical_nic and os_type_client == "linux":
            cmd = "ifconfig %s 0.0.0.0" % client_physical_nic
...
def start_test(
    ...
    netperf_base.record_env_version(
        test, params, host, server_ctl, fd, test_duration)
            ...
            for j in sessions_test:
                ...
                if ret:
                    ...
                    row, key_list = netperf_base.netperf_record(
                        ret, record_list,
                        header=record_header, base=base, fbase=fbase
                    )
                    category = ""
                    if record_header:
                        record_header = False
...
def launch_client(
    ...
    if _netserver_started:
        test.log.info("Netserver already started.")
    else:
        test.log.info("TEST_STEP: Ensure netserver is running on server side")
        ...
        if params.get("os_type") == "windows":
            ...
            test.log.info(
                "Start netserver on windows guest, cmd is: %s",
                netserv_start_cmd)
            ...
            netperf_base.ssh_cmd(
                server_ctl, "pidof netserver || %s" % server_path)
    ...
    def count_interrupt(name):
        ...
        sum = 0
        intr = []
        stat = netperf_base.ssh_cmd(
            server_ctl, "grep %s /proc/interrupts" % name)
    ...
    def get_state():
        ...
        cmd = (
            ...
            "%s/rx_bytes|xargs cat;%s/tx_bytes|xargs cat"
            % (path, path, path, path)
        )
        ...
        nre = int(
            netperf_base.ssh_cmd(
                server_ctl, "grep Tcp /proc/net/snmp|tail -1"
            ).split()[12]
        )
        state_list = [
            "rx_pkts",
        ...
        test.log.debug(f"get_state returning: {state_list}")
        return state_list
    ...
    def thread_cmd(params, i, numa_enable, client_s, timeout):
        ...
        option += " >> %s" % fname
        netperf_base.netperf_thread(
            params, numa_enable, client_s, option, fname)
    ...
    while tries > 0:
        ...
        if utils_misc.wait_for(
            ...
            finished_result = netperf_base.ssh_cmd(
                clients[-1], "cat %s" % fname)
