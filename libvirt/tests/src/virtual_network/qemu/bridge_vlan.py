...
def run(test, params, env):
    ...
    def set_ip_vlan(vlan_if, vlan_ip, session=None):
        ...
        if session:
            stop_firewall = params.get("stop_firewall", "")
            session.cmd_output_safe(stop_firewall)
            stop_nm = params.get("stop_nm", "")
            session.cmd_output_safe(stop_nm)
            session.cmd_output_safe("ip addr flush dev %s" % vlan_if)
            session.cmd_output_safe("ip link set dev %s down" % vlan_if)
            session.cmd_output_safe("ip addr add %s/24 dev %s" % (vlan_ip, vlan_if))
            session.cmd_output_safe("ip link set dev %s up" % vlan_if)
        else:
            _system("ip addr add %s/24 dev %s" % (vlan_ip, vlan_if))
            _system("ip link set dev %s up" % vlan_if)

    def set_mac_vlan(vlan_if, macs, session):
        """
        Give a new mac address for vlan interface in guest.
        :params: vlan_if: Vlan interface.
        :params: macs: New mac address for vlan.
        :params: session: VM session.
        """
        mac_cmd = "ip link set dev %s address %s up" % (vlan_if, macs)
        error_context.context(
            "Give a new mac address '%s' for vlan interface '%s'" % (macs, vlan_if),
            test.log.info,
        )
    ...
    def netperf_vlan(client="main_vm", server="localhost", sub_type="netperf_stress_test", session=None):
        """
        ...
        :params session: VM session for network restart.
        """
        ...
        if session and params.get_boolean("netperf_restart_nm"):
    ...
    for vm_name in params.get_list("vms"):
    ...
    for vm_index, vm in enumerate(vms):
        ...
        session.cmd_output_safe("ip addr flush dev %s" % ifname[vm_index])
        vlan_if = add_vlan(
            interface=ifname[vm_index], v_id=host_vlan_id, session=session
        )
        vm_vlan_if.append(vlan_if)
        set_mac_vlan(vlan_if, macs[vm_index], session=session)
    ...
    netperf_vlan(client=params["main_vm"], server=params.get("second_vm", "vm2"), session=sessions[0])

    exithandlers = "exithandlers__%s" % sub_type
    sub_exit_timeout = params.get_numeric("sub_exit_timeout", 10)
    start_time = time.time()
    end_time = start_time + sub_exit_timeout
