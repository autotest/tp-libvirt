...
def check_interface_in_vm_with_ip_l(vm, iface_mac):
    """
    VM-level interface verification using standard Linux networking tools.
    ...
    try:
        ...
        logging.debug("'ip l' output: %s", output)

        # Parse the output to find interface with matching MAC
        lines = output.split('\n')
        interface_name = None
        for i, line in enumerate(lines):
            if re.search(iface_mac.lower(), line.lower()):
                # Try to extract interface name from previous line
                if i > 0:
                    prev_line = lines[i-1]
                    match = re.search(r'^\d+:\s+(\S+):', prev_line)
                    if match:
                        interface_name = match.group(1)
                break

        if not re.search(iface_mac.lower(), output.lower()):
            return (1, "Interface with MAC %s not found in VM via 'ip l'" % iface_mac, None)

        logging.info("Interface %s found in VM via 'ip l' (interface: %s)",
...
def run(test, params, env):
    ...
    # Skip test if libvirt version doesn't support required flags
