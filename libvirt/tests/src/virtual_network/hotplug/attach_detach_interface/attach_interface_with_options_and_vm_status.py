...
def check_interface_xml(vm_name, iface_type, iface_source, iface_mac,
    ...
    try:
        ...
        logging.info("Checking %s XML for interface %s",
                     xml_type, iface_mac)
        xpaths_text = [
            {'element_attrs': [f".//interface[@type='{iface_type}']"]},
            {'element_attrs': [
                f".//interface/mac[@address='{iface_mac}']"]},
            {'element_attrs': [
                f".//interface/source[@network='{iface_source}']"]}
        ]
        result = libvirt_vmxml.check_guest_xml_by_xpaths(
            vmxml, xpaths_text, ignore_status=True)
        logging.info("Interface %s %s in %s XML", iface_mac,
                     "found" if result else "not found", xml_type)
        return result
    except Exception as e:
        logging.error(
            "Failed to check interface XML for VM '%s': %s", vm_name, str(e))
        raise
...
def check_interface_in_vm_with_ip_l(vm, iface_mac):
    ...
    try:
        with vm.wait_for_login() as session:
            ...
            interface_name = utils_net.get_linux_ifname(
                session, iface_mac)
        ...
        error_msg = "Failed to find interface in VM: %s" % str(e)
        logging.error(
            "Failed to find interface with MAC %s: %s", iface_mac, str(e))
...
def run(test, params, env):
    ...
    try:
        ...
        logging.info(
            "Test completed successfully for flags: %s", test_flags)
