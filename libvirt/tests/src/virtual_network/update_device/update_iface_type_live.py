...
def run(test, params, env):
    ...
    try:
        ...
        if operation == 'attach_device':
            virsh.attach_device(vm_name, iface.xml, flagstr='--persistent', **VIRSH_ARGS)
        elif operation == 'attach_interface':
            virsh.attach_interface(vm_name, f'--source {net_name} --model virtio --type network --persistent', **VIRSH_ARGS)
