...
def run(test, params, env):
    ...
    6) Retouch the file to ensure guest file systems are not frozen;
    ...
    if not virsh.has_help_command("domfsfreeze"):
        test.cancel(
            "This version of libvirt does not support the domfsfreeze/domfsthaw test"
        )
    ...
    try:
        ...
        mountpoints = ["/"]
        if require_second_disk:
            second_mntpoint = mount_second_disk_in_vm(session)
            mountpoints.append(second_mntpoint)
