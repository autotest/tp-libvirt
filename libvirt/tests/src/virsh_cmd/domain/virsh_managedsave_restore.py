...
def run(test, params, env):
    ...
    def setup_saved_file():
        ...
        if file_state == "corrupt":
            ...
            virsh.managedsave_define(vm_name, xmlfile, **VIRSH_ARGS)
            ret = virsh.managedsave_dumpxml(vm_name, '--xpath //disk', **VIRSH_ARGS).stdout_text
    ...
    try:
        ...
        ret = virsh.start(vm_name, **VIRSH_ARGS)
        libvirt.check_result(ret, expected_fails=expected_cmd_err)
        virsh.destroy(add_vm_name, **VIRSH_ARGS)
        ...
        virsh.undefine(vm_name, "--managed-save --keep-nvram", **VIRSH_ARGS)
        if os.path.exists(managed_save_file):
            process.run("rm -f %s" % managed_save_file, shell=True, ignore_status=True)
        bk_xml.sync()
        if need_vm2 and add_vm_name != 'vm2':
