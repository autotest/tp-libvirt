...
def get_storage_devices():
    ...
    try:
        ...
        if not os.path.exists(storage_path):
            logging.debug("Storage device path %s doesn`t exists!", storage_path)
            return []
            ...
            if dev_type and not dev_real_virtual:
                if dev_type.group(0) == "disk":
                    ...
                    if dev_id:
                        ...
                        dev_name = re.sub(r"\W", "_", "block_%s_%s" % (device, serial))
...
def get_ccw_io_devices():
    ...
    for device in os.listdir(ccw_base_path):
        ...
        online_value = process.run(
            "cat %s" % online_attribute, timeout=5, ignore_status=True, shell=True
        ).stdout_text
    ...
    if cap_option == "one":
        ...
        for cap in caps:
            ...
            if cap == "storage" and remove_scm_device:
                ...
                if scm_devices:
                    devices[cap] = [
                        device for device in devices[cap] if device not in scm_devices
                    ]
            if not _check_result(
                cap, devices[cap], result.stdout.strip(), mode, pattern
            ):
                check_failed = True
                break
    else:
