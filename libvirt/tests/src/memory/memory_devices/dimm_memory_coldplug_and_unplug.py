...
def run(test, params, env):
    ...
    def setup_test():
        """
        Modify the config params accroding to test environment
        """
        def _clean_empty_value_dict(config_dict):
            """
            Clean empty config of dictionary

            :param config_dict: the dictionary to be cleaned
            """
            for key in list(config_dict.keys()):
                if not config_dict[key]:
                    del config_dict[key]

        def _form_data_struct(config_string, page_size):
            """
            Process config dictionary string to python data structures
            ...
            """
            return (
                eval(config_string % page_size)
                if test_case == "with_target_source"
        ...
        init_dimm_dict = _form_data_struct(init_dimm_dict, page_size)
        init_xpath_list = _form_data_struct(init_xpath_list, page_size)
        _clean_empty_value_dict(init_dimm_dict)
        if with_plug_dimm:
            plug_dimm_dict = _form_data_struct(plug_dimm_dict, page_size)
            plug_xpath_list = _form_data_struct(plug_xpath_list, page_size)
            _clean_empty_value_dict(plug_dimm_dict)

    def check_dimm_mem_device_xml(*dimm_xpaths):
        def _match(dimm_mem, xpath_list):
        ...
        for memory_device in memory_devices:
            ...
            for xpath_list in dimm_xpaths:
                if xpath_list not in found_xpath and _match(memory_device, xpath_list):
                    found_xpath.append(xpath_list)
                    found = True
                    break
            if not found:
                test.fail("Expected xpath is %s, but found dimm mem device %s" % (
                    xpath_list, memory_device))
