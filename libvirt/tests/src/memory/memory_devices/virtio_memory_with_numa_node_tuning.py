...
def get_two_hugepage_sizes(test, params):
    ...
    max(hugepage_sizes), the max hugepage size that is closed to dividend_size and
    different from default hugepage size.
    """
    dividend_size = params.get("dividend_size", 1048576)
...
def consume_guest_mem(vm, test):
    ...
    if not vm.is_alive():
        vm.start()
    test.log.debug("The guest xml is:\n%s",
...
def _get_left_mem(vm, params, test, host_numa_node, hp_size):
    ...
    for mem in vmxml.devices.by_device_tag('memory'):
        ...
        if mem_attrs.get("source"):
            used_host_numa_node = int(mem_attrs.get("source")['nodemask'])
            used_pagesize = int(mem_attrs.get("source")['pagesize'])
        else:
