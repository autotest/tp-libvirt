...
def run(test, params, env):
    ...
    def check_device_on_qemu_cmd_line():
        ...
        pattern = fr'-device\s+{{"driver":"({memballoon_driver})".*?"id":"({memballoon_alias_name})".*?}}'
    ...
    def consume_guest_memory(guest_session=None, close_session=False):
        ...
        (status, _) = libvirt_memory.consume_vm_freememory(guest_session, memory_to_consume)
        if status:
            test.fail(f"Memory consume {memory_to_consume} failed")
        if close_session:
            guest_session.close()
        return guest_session
