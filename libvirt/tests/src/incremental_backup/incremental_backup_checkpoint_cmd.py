...
def run(test, params, env):
    ...
    def edit_checkpoint(vm_name, checkpoint_name, restore=False):
        ...
        try:
            ...
            session.close()
            edit_result = virsh.checkpoint_dumpxml(vm_name, checkpoint_name,
                                                   "--no-domain").stdout_text.strip()
            test.log.debug("The checkpoint dumpxml after edit: %s", edit_result)
            if not restore:
                if edit_result.count("bitmap") == 4:
                    test.log.debug("Edit checkpoint succeeded!")
                else:
                    test.fail("Edit checkpoint failed!")
        except (aexpect.ShellError, aexpect.ExpectError) as details:
            log = session.get_output()
            session.close
            test.fail("Edit checkpoint failed: %s\n%s" % (details, log))
