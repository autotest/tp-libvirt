...
def run(test, params, env):
    ...
    """
    def create_disk_images(disk_dict, new_image_path, prepare_disk_xml=False):
        """
    ...
    def prepare_guest():
        ...
        """
        disk_dict = eval(params.get("disk_dict", "{}") % "vdb")
        new_image_path = data_dir.get_data_dir() + '/vdb.qcow2'
        _, disk_image_list = create_disk_images(disk_dict, new_image_path)
        if not with_hotplug:
            disk_dict = eval(params.get("disk_dict", "{}") % "vdc")
    ...
    def backup_begin(backup_dict, checkpoint_dict):
        """
        ...
        test.log.debug("The checkpoint xml is: %s" % checkpoint_dict)
        ...
        if "Backup" not in res:
            test.fail("The Backup operation isn't in the domjobinfo as expect.")
    ...
    def check_bitmaps():
        ...
        if sorted(vdc_bitmaps) != sorted(checkpoint_list):
    ...
    try:
        ...
        if with_hotplug:
            full_backup_dict = eval(params.get("full_backup_dict") % backup_file_list[0])
            inc_backup_dict = eval(params.get("inc_backup_dict") % (backup_file_list[1], backup_file_list[2]))
            diff_backup_dict = eval(params.get("inc_backup_dict") % (backup_file_list[3], backup_file_list[4]))
            backup_configs = [
                (full_backup_dict, eval(params.get("first_checkpoint_dict") % checkpoint_list[0])),
                (inc_backup_dict, eval(params.get("next_checkpoint_dict") % checkpoint_list[1])),
                (diff_backup_dict, eval(params.get("next_checkpoint_dict") % checkpoint_list[2])),
            ]
            for backup_dict, checkpoint_dict in backup_configs:
                if backup_dict == inc_backup_dict:
                    disk_dict = eval(params.get("disk_dict") % "vdc")
                    new_image_path = data_dir.get_data_dir() + '/vdc.qcow2'
                    disk_xml, disk_image_list = create_disk_images(disk_dict, new_image_path, prepare_disk_xml=True)
                    virsh.attach_device(vm_name, disk_xml.xml, debug=True, ignore_status=False)
                backup_begin(backup_dict, checkpoint_dict)
            check_bitmaps()
        else:
            full_backup_dict = eval(params.get("full_backup_dict") % (backup_file_list[0], backup_file_list[1]))
            inc_backup_dict = eval(params.get("inc_backup_dict") % backup_file_list[2])
            diff_backup_dict = eval(params.get("full_backup_dict") % (backup_file_list[3], backup_file_list[4]))
            backup_configs = [
                (full_backup_dict, eval(params.get("first_checkpoint_dict") % checkpoint_list[0])),
                (inc_backup_dict, eval(params.get("next_checkpoint_dict") % checkpoint_list[1])),
                (diff_backup_dict, eval(params.get("first_checkpoint_dict") % checkpoint_list[2])),
            ]
            for backup_dict, checkpoint_dict in backup_configs:
                vm_session = vm.wait_for_login()
                new_disk = libvirt_disk.get_non_root_disk_name(vm_session)[1]
                if backup_dict == inc_backup_dict:
                    libvirt_disk.check_virtual_disk_io(vm, new_disk, umount=False)
                backup_begin(backup_dict, checkpoint_dict)
            _, output = vm_session.cmd_status_output("ls -l /mnt")
