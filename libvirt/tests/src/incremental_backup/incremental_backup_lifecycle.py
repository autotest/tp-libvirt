...
def test_shutdown_vm(test, params, backup_file_list):
    ...
    def check_blockjob_progress(vm_name, target_disk, previous_progress, stage_name):
        """
    ...
    if not utils_misc.wait_for(
        lambda: int([x for x in virsh.blockjob(vm_name, target_disk, options="--raw", debug=True,
                                               ignore_status=False).stdout_text.split()
                    if x.startswith("cur=")][0].split("=")[1]) > 0, timeout=5):
        test.fail("The blockjob is still not running within timeout.")
    ...
    if not utils_misc.wait_for(lambda: "paused" in virsh.domstate(vm_name).stdout_text, 10):
        test.fail("Can't get the expected guest status: paused.")
    paused_progress = check_blockjob_progress(vm_name, target_disk,
    ...
    if resume_after_shutdown:
        ...
        if not utils_misc.wait_for(lambda: "running" in virsh.domstate(vm_name).stdout_text, 10):
            test.fail("Can't get the expected guest status: running.")
        check_blockjob_progress(vm_name, target_disk, paused_progress, "after resume")
    else:
        test.log.info("Abort the blockjob and check the guest status.")
        virsh.domjobabort(vm_name, debug=True, ignore_status=False)
        if not utils_misc.wait_for(lambda: "shut off" in virsh.domstate(vm_name).stdout_text, 10):
            test.fail("Can't get the expected guest status: shut off.")
