...
def run(test, params, env):
    ...
    def check_internal_snap():
        ...
        internal_snap_list = utils_misc.get_image_snapshot(disk["source"])
    ...
    def check_disk_xml(xpath_list):
        ...
        :param xpath: expected xpath list.
        """
    ...
    def check_file_after_snapshot_delete(file, exist):
        ...
        :param file: snapshot related file path.
        :param exist: bool, file should exist or not.
        """
        if os.path.exists(file) != exist:
            judgement_string = "" if exist else "not "
            test.fail("file '%s' should %sexit after snapshot delete." % (file, judgement_string))
    ...
    def run_test():
        ...
        """
        test.log.info("TEST_STEP1:Define a guest.")
        vmxml.sync()
        ...
        dom_xml = vm_xml.VMXML.new_from_dumpxml(vm_name)
        current_disk = list(dom_xml.get_disk_all().values())[0]
        current_disk_xml = ElementTree.tostring(current_disk, encoding='unicode')
        original_disk_xml = ElementTree.tostring(original_disk, encoding='unicode')
        if original_disk_xml != current_disk_xml:
            test.fail("Expected domain disk is %s, but found %s." %
                      (original_disk_xml, current_disk_xml))
        ...
        for snap_file in external_snap_file_list:
            check_file_after_snapshot_delete(snap_file, False)
        check_file_after_snapshot_delete(disk["source"], True)
    ...
    external_snap_option = params.get("external_snap_option")
    internal_snap_name = params.get("internal_snap_name")
    external_snap_name = params.get("external_snap_name")
    external_mem_file = params.get("external_mem_file")

    vm_name = guest_os_booting_base.get_vm(params)
    vmxml = vm_xml.VMXML.new_from_inactive_dumpxml(vm_name)
    bkxml = vmxml.copy()
    original_disk = list(vmxml.get_disk_all().values())[0]
    vm = env.get_vm(vm_name)
    disk = vm.get_first_disk_devices()
    ori_image_file, _ = os.path.splitext(disk["source"])
    ...
    disk_xpath = eval(params.get("disk_xpath") % (external_snapshot_file, disk["source"]))
