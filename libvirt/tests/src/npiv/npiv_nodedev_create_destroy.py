...
def find_hbas(hba_type="hba", status="online"):
    ...
    """
#TODO: add offline/online judgement, fc storage not stable for now, so leave this part after we buy npiv server
    ...
    for scsi_host in scsi_hosts:
        ...
        if result.exit_status:
            raise exceptions.TestFail(result.stderr)
        if re.search('vport_ops', result.stdout.strip()) and not re.search('<fabric_wwn>ffffffffffffffff</fabric_wwn>', result.stdout.strip()):
            online_hbas_list.append(scsi_host)
        #  if re.search('fc_host', result.stdout.strip()) and not re.search('vport_ops', result.stdout.strip()) and not re.search('<fabric_wwn>ffffffffffffffff</fabric_wwn>', result.stdout.strip()):
        if re.search('fc_host', result.stdout.strip()) and not re.search('vport_ops', result.stdout.strip()):
...
def vhbas_cleanup():
    ...
    if left_vhbas:
        logging.error("some vhbas not cleaned: %s", left_vhbas)
    logging.debug("all scsi_hosts destroied: %s", vhbas)
...
def run(test, params, env):
    ...
    set_valid_wwn_to_first_vhba = "yes" == params.get("set_valid_wwn_to_first_vhba", "no")
    scsi_wwnn = params.get("scsi_wwnn", "")
    scsi_wwpn = params.get("scsi_wwpn", "")
    online_hbas = []
    ...
    try:
        for i in range(create_number):
            #steps in case 4: create some vhbas, and first vhba is valid, rest vhbas with random wwns
            ...
            if set_valid_wwn_to_first_vhba and i == 0:
                ...
                new_vhba = nodedev_create_from_xml({"nodedev_parent": first_online_hba,
                                                    "scsi_wwnn": scsi_wwnn,
                                                    "scsi_wwpn": scsi_wwpn})
                utils_misc.wait_for(lambda: is_vhbas_added(old_vhbas), timeout=5)
            ...
            #set wwn (valid or random depending on test case variables) to vhbas
            else:
                old_vhbas = find_hbas("vhba")
                new_vhba = nodedev_create_from_xml({"nodedev_parent": first_online_hba,
                                                    "scsi_wwnn": scsi_wwnn,
