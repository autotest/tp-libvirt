...
from virttest import virsh                                         # pylint: disable=import-error
from virttest import utils_test, utils_misc                        # pylint: disable=import-error
from virttest.libvirt_xml.vm_xml import VMXML                      # pylint: disable=import-error
from virttest.libvirt_xml.nodedev_xml import NodedevXML            # pylint: disable=import-error
from virttest.test_setup import PciAssignable                      # pylint: disable=import-error
from virttest.libvirt_xml.devices.controller import Controller     # pylint: disable=import-error
from virttest import utils_package                                 # pylint: disable=import-error
from virttest import libvirt_version                               # pylint: disable=import-error
    ...
    def test_eeh_nic():
        ...
        for num_of_hit in range(1, int(max_freeze) + 2):
            if num_of_miss < 5:
                ...
                if eeh_guest == "yes":
                    test.cancel("The Feature is still under Development, Will enable the Test once the Feture is ready in production")
                    session.cmd(eeh_cmd)
                if eeh_host == "yes":
