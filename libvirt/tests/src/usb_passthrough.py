...
def run(test, params, env):
    ...
    def is_USB(device_type):
        usbs = utils.run("lsusb", timeout=10,ignore_status='False', verbose=False)
	#findusb = subprocess.Popen('lsusb',shell=True,stdout=subprocess.PIPE)
    ...
    existing_usb = utils.run("lsusb", timeout=10,ignore_status='False', verbose=False)
    #findusb = subprocess.Popen('lsusb',shell=True,stdout=subprocess.PIPE)
	...
	pass_usbs=[]
		...
		if ((usb.split(' '))[5]) in lsusb_op_bef:
		    logging.info("%s already inuse within guest.. skipping",((usb.split(' '))[5]))
		    continue
	...
	if pass_usbs =='':
		...
		if ((usb.split(' '))[5]) in lsusb_op_bef:
		    logging.info("%s already inuse within guest.. skipping",usb)
	    	else:
		    pass_usbs.append(usb)
    ...
    if pass_usbs == []:
        raise error.TestNAError("Either there are no usbs  available for guest passthrough or already in use within guest.. skipping..")
    logging.info("Passthrough will occur for following USB devices : %s", pass_usbs)
    pass_reported = []
    fail_reported = []
    failures_reported = 0
    for usb_dev in pass_usbs:
        ...
        if not vm.is_alive():
            vm.start()
	logging.info("USB devices within guest before passthrough: %s", lsusb_op_bef)
        # Edit guest xml to add hostdev entries for diffrent ports
        usb_address = {'bus':'0x'+(usb_dev.split(' '))[1],'device':'0x'+((usb_dev.split(' '))[3]).strip(':'),'vendor_id': '0x'+(((usb_dev.split(' '))[5]).split(":"))[0], 'product_id': '0x'+(((usb_dev.split(' '))[5]).split(":"))[1]} 
	    ...
	    timeout = float(params.get("login_timeout", 240))
            ...
            logging.info ("USB devices within guest after passthrough: %s"%str(lsusb_op_aft))
            if lsusb_op_bef == lsusb_op_aft:
	        failures_reported = 1	
	        logging.info ("Passthrough failed for USB device %s"%str(usb_dev))
		fail_reported.append(usb_dev)
	    else:	
                logging.info ("Passthrough passed for USB device %s"%str(usb_dev))
    ...
    if failures_reported:
    	raise error.TestFail("USB device passthrough failed for one or more devices, see above output for more details")
