...
def mount_in_vm(vm, mount_src, mount_dir):
    """
    Mount in vm
...
def prepare_both_vms(params, vm1, vm2, vm1_xml, vm2_xml, test):
    ...
    vm2_xml = update_cpu_xml(vm2_xml, cpu_mode, test, l1vm_feature_list)

    prepare_package_in_vm(vm1)
    prepare_package_in_vm(vm2)

    prepare_env_in_vm(vm1, vm1_hostname, mount_src, mount_dir, desturi_port)
    prepare_env_in_vm(vm2, vm2_hostname, mount_src, mount_dir, desturi_port)
    ...
    mount_in_vm(vm2, (vm1_ip + ":" + mount_src), mount_dir)

    vm1_session = vm1.wait_for_login()
    vm2_session = vm2.wait_for_login()
    hosts_dict = {"%s" % vm1_hostname: "%s" % vm1_ip, "%s" % vm2_hostname: "%s" % vm2_ip}
    utils_net.map_hostname_ipaddress(hosts_dict, vm1_session)
...
def run(test, params, env):
    ...
    def setup_test():
        ...
        prepare_both_vms(params, vm1, vm2, vm1_xml, vm2_xml, test)
