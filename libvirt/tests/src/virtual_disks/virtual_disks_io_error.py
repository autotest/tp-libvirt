...
def run(test, params, env):
    ...
    def run_test():
        ...
        _, output = session.cmd_status_output(dd_in_guest)
        if dd_msg not in output:
            test.fail("Write data in guest should produce error: %s in %s" % (dd_msg, output))

        test.log.info("TEST_STEP2: Check event output.")
        ...
        test.log.info("TEST_STEP3: Check about I/O error in vm log.")
        ...
        test.log.info("TEST_STEP4: Check about I/O error in virsh domain info.")
