...
def get_size_for_rbd(new_image_path, mon_host, server_user, server_pwd, test):
    ...
    Args:
        new_image_path: Path to the RBD image
        mon_host: Monitor host for Ceph cluster
        server_user: SSH username
        server_pwd: SSH password
        test: Test object for logging and error handling

    Returns:
        int: Used disk size in MB
    """
            ...
            if image_name in line:
                ...
                if len(parts) >= 5:
                    try:
                        ...
                        test.log.debug("Extracted used disk size: %s MB", disk_size)
                        return disk_size
                    except (ValueError, IndexError):
                        test.fail("Failed to parse disk size from rbd du output: %s" % parts[3])

        test.fail("Image %s not found in rbd du output" % image_name)
    finally:
        ceph_session.close()
...
def run(test, params, env):
    ...
    def write_data(session):
        ...
        check_disk = session.cmd_output("lsblk -M").strip()
        ...
        test.log.info("Wait for 10 seconds to make sure the data has been written.")
    ...
    def compare_disk_size():
        ...
        test.log.info("Wait for 20 seconds to make sure the disk size has been discard.")
        time.sleep(25)
