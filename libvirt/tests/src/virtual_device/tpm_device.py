...
def get_supported_values():
    ...
    """
    out_m = process.run('/usr/libexec/qemu-kvm -device help').stdout_text.strip()
    tpm_mdls = re.findall('tpm-[a-z]+', out_m)
    out_b = process.run("/usr/libexec/qemu-kvm -tpmdev help", ignore_status=True).stderr_text.strip().split('\n')
    del out_b[0]
    tpm_bkds = []
    for b in out_b:
        b_list = b.split(' ')
...
def run(test, params, env):
    ...
    if domcaps_check:
        expect_values = get_supported_values()
