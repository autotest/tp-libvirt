...
def run(test, params, env):
    ...
    def check_process_exist(process_cmd):
        """
        Check if a cmd process exists in system processes.
        """
        if process_cmd == swtpm_socket_cmd:
            check_cmd = "ps aux|grep 'swtpm socket'|grep -v avocado-runner-avocado-vt|grep -v grep"
        elif process_cmd == qemu_cmd:
            check_cmd = "ps aux|grep '/usr/libexec/qemu-kvm -tpmdev'|grep -v avocado-runner-avocado-v|grep -v grep"
    ...
    try:
        if co_cmd:
            ...
            p_out = subprocess.Popen(co_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).stdout.read()
            logging.debug("The cocurrent swtpm_setup output including 'read' is:\n%s", p_out)
            ...
            if qemu_cmd and check_process_exist(swtpm_socket_cmd):
                ...
                elif not swtpm_socket_exists and '--terminate' not in swtpm_socket_cmd:
                    test.fail("swtpm process should not exit without '--terminate' when qemu is killed.")

    finally:
        logging.debug('Cleaning env...')
        if check_process_exist(swtpm_socket_cmd):
            kill_process('swtpm')
