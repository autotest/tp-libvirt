...
def run(test, params, env):
    ...
    def generate_expected_process_option():
        ...
        if openfiles:
            expected_results += " --rlimit-nofile(\s|=)%s" % open_files
    ...
    try:
        ...
        for index in range(fs_num):
            ...
            if launched_mode == "auto":
                binary_keys = ['path', 'cache_mode', 'xattr', 'thread_pool_size', "open_files"]
                binary_values = [path, cache_mode, xattr, thread_pool_size, open_files]
