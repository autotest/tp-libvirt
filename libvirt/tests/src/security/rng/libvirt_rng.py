...
def run(test, params, env):
    ...
    def check_guest(session, expect_fail=False, set_virtio_current=False):
        ...
        if rng_rate:
            ...
            if rate_real > rate_conf * 1.2:
                test.fail(
                    "The rate of reading exceed the limitation of configuration"
                )
        if device_num > 1:
    ...
    def handle_hotplug():
        ...
        if attach_rng:
            ...
            if start_error:
                test.fail("VM must be running for hotplug")
        return ""
    ...
    if device_num > 1 and not libvirt_version.version_compare(1, 2, 7):
        test.cancel(
            "Multiple virtio-rng devices not supported on this libvirt version"
        )
