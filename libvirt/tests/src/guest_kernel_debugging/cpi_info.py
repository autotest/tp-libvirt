...
def run(test, params, env):
    ...
    if not utils_misc.compare_qemu_version(10, 1, 0, is_rhev=False):
        test.cancel("CPI tests require QEMU version 10.1.0 or later")
    ...
    def test_managedsave():
        ...
        final_timestamp = final_cpi_data["timestamp"]

        logging.info(f"Final CPI data: {final_cpi_data}")
        logging.info(f"Final timestamp: {final_timestamp}")

        # Verify all fields remain the same
        for field in ["system_type", "system_name", "sysplex_name", "system_level"]:
        ...
        # Verify timestamp remains the same (no reboot after managedsave)
        if initial_timestamp != final_timestamp:
            test.fail(
                f"Timestamp changed after managedsave: "
    ...
    def test_long_system_name():
        ...
        long_system_name = "VERYLONGSYSTEMNAME123456789"
        logging.info(
