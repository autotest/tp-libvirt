...
def run(test, params, env):
    ...
    def crash_utility(dump_file, vm):
        ...
        returns error
        If crash tool cannot read into the vmcore, the function returns fail
        If crash tool can read the vmcore, the function returns true
        ...
        try:
            ...
            # Send the 'bt' command and capture output
            child.sendline("bt | head")
            child.expect("crash> ")
            stdout = child.before.decode('utf-8')

            # Check if the backtrace produced output
