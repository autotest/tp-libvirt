...
def run(test, params, env):
    ...
    def check_guest_runnning_after_virtlogd_crash():
        ...
        process.run(f'kill -9 {before_pid_virtlogd}', shell=True, verbose=True, ignore_status=True)
    ...
    def set_max_size_in_virtlogd_config():
        ...
        action = params.get("action")
        process.run("systemctl %s virtlogd" % action, ignore_status=True, verbose=True, shell=True)
    ...
    def check_reload_max_size_config():
        ...
        output = process.run(ls_file_size_cmd, ignore_status=True, verbose=True, shell=True).stdout_text
    ...
    def check_set_max_backups():
        ...
        while time_start <= time_end:
            ...
            number = process.run(ls_logfile_number_cmd, ignore_status=True, verbose=True, shell=True).stdout_text.strip()
