...
def check_controller_by_xml(test, params, vm_name, target_cntl):
    """
    ...
    new_memReserve = params.get("new_memReserve")
    cntl = get_controller(test, cur_vm_xml, target_cntl)
    actual_memReserve = cntl.target.get("memReserve")
    if not actual_memReserve or int(actual_memReserve) != int(new_memReserve):
        test.fail("Expect memReserve to be %s, "
                  "but found %s" % (new_memReserve, actual_memReserve))
    else:
...
def run(test, params, env):
    ...
    new_memReserve = int(params.get("new_memReserve"))
    ...
    try:
        ...
        check_controller_by_xml(test, params, vm_name, target_cntl)
        ...
        if actual_reserve_size != new_memReserve:
            test.fail("Expect mem reserve size "
                      "in vm to be %s, but found %s" % (new_memReserve,
                                                        actual_reserve_size))
        else:
            test.log.debug("Verify: The mem reserve size "
                           "in vm is %s as expected - PASS", actual_reserve_size)
        session.close()
    finally:
        vm_xml_backup.sync()
