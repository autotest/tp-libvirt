...
def run(test, params, env):
    ...
    try:
        ...
        LOG.debug(f"Step4: Check the VM's state details after save abort")
        outputs_ = virsh.domstate(vm_name, "--reason").stdout_text.strip()
        LOG.debug(f"vm state detail is %s", outputs_)
