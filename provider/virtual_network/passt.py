...
def generate_random_ip_addr(exclude_multicast_addr=False):
    """
    Generate random ip address, optionally excluding multicast addresses (224.0.0.0 - 239.255.255.255)

    :param exclude_multicast_addr: if True, exclude multicast addresses
    :return: random ip address
    """
    if exclude_multicast_addr:
        first_octet = random.choice(list(range(224)) + list(range(240, 256)))
    else:
        first_octet = random.randint(0, 255)

    digits = [str(first_octet)] + [str(random.randint(0, 255)) for i in range(3)]
    return '.'.join(digits)
