...
def check_vm_ip(iface_attrs, session, host_iface, vm_iface=None):
    ...
    if ipv6_info:
        ...
        LOG.warning("Host ipv6 is not available! Skip the test")
        raise exceptions.TestCancel(f'Host ipv6 is not available!')
    if 'ips' in iface_attrs:
...
def generate_random_ip_addr():
    """
    Generate random ip address

    :return: ip address generated by function
    """
    digits = [str(random.randint(0, 255)) for i in range(4)]
    ip = '.'.join(digits)
    return ip
